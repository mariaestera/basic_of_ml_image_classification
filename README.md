...
The purpose was to train and test different models at available digit images 
and then apply them to unseen data -  **larger numbers** (unknown length) written by myself**.
...
2) **Multilayer Perceptrons** (MLP)
    for each model different hidden layer sizes was compared. 
...
Next, the frequency of each digit (and the distribution of number lengths) were analyzed. (`data_labelling.ipynb` script)
   - first step of validation workflows was to prepare digit to the "mnist" format: 28x28 images with centered digits in gey scale:
